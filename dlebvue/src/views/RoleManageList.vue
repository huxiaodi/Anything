  ...
  <div>
    ...
    <el-form :inline="true" :model="searchForm" class="admin-search-form" align="left">
      ...
      <div class="admin-search-btns">
        ...
        <el-button type="primary" icon="el-icon-plus" @click="addBefore">新增</el-button>
        <el-button type="primary" icon="el-icon-edit" @click="editBefore">修改</el-button>
        <el-button type="primary" icon="el-icon-delete" @click="deleteRole">删除</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取数据
      getData() {
        api.post('admin/roleManage/roleList', this.searchForm).then(response => {
      ...
      addBefore() {
        ...
        api.get('admin/roleManage/allRoleResources').then(response => {
          if (response.data.code === 1) {
      ...
      addRole(formName) {
        ...
        api.post('admin/roleManage/addRole', this.form).then(response => {
          if (response.data.code === 1) {
      ...
      editRole(formName) {
        ...
        for (var i = 0; i < this.$refs.changetree.getCheckedKeys().length; i++) {
          this.test.resourceIds[i] = this.$refs.changetree.getCheckedKeys()[i]
        }
