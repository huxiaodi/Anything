<template>
  <div>
    ...
    <el-table ref="myTable" stripe border
      ...
      <el-table-column prop="kindCode" sortable align="center" width="110px" label="品种代码"></el-table-column>
      <el-table-column prop="kindName" sortable align="center" width="230px" label="品种"></el-table-column>
    ...
    <!--品种字典管理新增修改-->
    <el-dialog
      :title="titleType=='add'?'新增品种字典':'修改品种字典'"
      ...
      <el-form ref="dialogForm" :rules="dialogFormRules" :model="dialogForm"
        ...
        <el-form-item label="品种代码:" prop="kindCode">
          <el-input type="text" v-model="dialogForm.kindCode" clearable :disabled="titleType == 'edit'"></el-input>
        </el-form-item>
        <el-form-item label="品种:" prop="kindName" style="margin-top: 16px">
          <el-input type="text" v-model="dialogForm.kindName" clearable
                                 v-on:change="checkKey(dialogForm.kindName)"></el-input>
  ...
  export default {
    ...
    data() {
      return {
        ...
        dialogFormRules: {
          kindCode: [
            {required: true, message: '请输入品种代码', trigger: 'blur' }
          ],
          kindName: [
    ...
    methods: {
      ...
      getData() {
        ...
        api.post('kindDict/list', this.searchForm).then(response => {
      ...
      //检查主键
      checkKey(keyValue) {
        api.get('kindDict/checkKey/' + keyValue).then(response => {
          if (response.data.code === 1) {
      ...
      deleteData() {
        if (this.row == null || this.row.length <= 0) {
          ...
          this.$confirm('确定删除该数据吗？', {
            ...
            api.post('kindDict/deletes', ids).then(response => {
              if (response.data.code === 1) {
      ...
      confirmDialogForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            api.post('kindDict/save', this.dialogForm).then(response => {
              if (response.data.code === 1) {
                Message.MessageSuccess(response.data.msg);
